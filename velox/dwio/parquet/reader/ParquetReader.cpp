...
bool parquetChildIsListStructEnvelopeIndexMode(
  ...
  return parquetChild.num_children ==
      static_cast<int32_t>(tableRow->size());
}
...
std::unique_ptr<ParquetTypeWithId> ReaderBase::getParquetColumnInfo(
  ...
  if (!schemaElement.__isset.type) { // inner node
    ...
    for (int32_t i = 0; i < schemaElement.num_children; i++) {
      ...
      if (!requestedType && parentRequestedType) {
        if (parentRequestedType->isArray() &&
          ...
          childRequestedType = parentRequestedType->asArray().elementType();
            ...
            parentRequestedType->isMap()) {
          const auto mapType = parentRequestedType->asMap();
          childRequestedType =
              i == 0 ? mapType.keyType() : mapType.valueType();
        }
      }
      ...
      RowTypePtr tableRow =
          requestedType && requestedType->isRow()
      ...
      const bool listStructEnvelope = parquetChildIsListStructEnvelopeIndexMode(
          ...
          parquetChild);
      if (listStructEnvelope &&
          childRequestedType == tableRow->childAt(i)) {
      ...
      if (childRequestedType && childRequestedType->isRow()) {
        ...
        const bool groupHoldsWholeStructRow =
        ...
        if (mapChildrenByName) {
          ...
          const bool arrayRowListBagWrapper =
              requestedType && requestedType->isArray() &&
              requestedType->asArray().elementType()->isRow() &&
              parquetChildIsGroup && parquetChild.__isset.num_children &&
